...
app.post('/login', (req, res) => {
    ...
    Usuario.findOne({ email: body.email }, (err, usuarioDB) => {
        ...
        if (bcrypt.compareSync(body.password, usuarioDB.password)) return res.status(500).json({ ok: false, err: { message: 'Usuario o contraseña incorreta' } });
    ...
    Usuario.findOne({ email: googleUser.email }, (err, usuarioDB) => {
        ...
        if (usuarioDB) {
            if (usuarioDB.google === false) {
                if (err) return res.status(400).json({ ok: false, err: { message: 'Debe usar su autenticacion normal' } });
            } else {
                let token = jwt.sign({ usuario: usuarioDB }, process.env.SEED, { expiresIn: process.env.CADUCIDAD_TOKEN });
                res.json({ ok: true, usuario: usuarioDB, token });
            }
